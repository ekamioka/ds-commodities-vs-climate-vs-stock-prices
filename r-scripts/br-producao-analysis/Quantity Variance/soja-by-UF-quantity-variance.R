##
## Question: Does the soy bean production by UF increased or decreased since 2000 until 2015?
##
...
dt = as.data.table(read.csv("../../../data/BR - producao/soja//producao_soja_uf.csv"
                            , sep = ","
                            , header = TRUE
                            , dec = "."))
...
for(i in 1:length(regioes)){
        ...
        # Plotting Produtividade de Soja por Região ("Produtividade" / "regiao")
        ...
        qplot(Safra
              ...
              , ylab = "Produtividade (Ton / ha)") + 
                geom_line(position=pd,aes(group=UF)) +
                geom_point() +
                ggtitle(paste(regioes_names[[i]]," - Produtividade de Soja por Região - Safras de 2000/01 a 2014/15"))
        
        ggsave(filename = paste(regioes_names[[i]],"-Produtividade.png", sep = ""), device = png, width = 1200, height = 500, limitsize = FALSE)
        
        # Plotting Produção de Soja por Região ("Producao" / "regiao")
        ...
        qplot(Safra
              ...
              , ylab = "Produção (milhões de toneladas)") + 
                geom_line(position=pd,aes(group=UF)) +
                geom_point() +
                ggtitle(paste(regioes_names[[i]]," - Produção de Soja por Região - Safras de 2000/01 a 2014/15"))
