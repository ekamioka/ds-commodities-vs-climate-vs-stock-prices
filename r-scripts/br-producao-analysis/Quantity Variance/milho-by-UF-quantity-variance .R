...
dt = as.data.table(read.csv("../../../data/BR - producao/milho/producao_milho_uf.csv"
                            , sep = ","
                            , header = TRUE
                            , dec = "."))
...
for(i in 1:length(regioes)){
        ...
        qplot(Safra
              ...
              , ylab = "Produtividade (Ton / ha)") + 
                geom_line(position=pd,aes(group=UF)) +
                geom_point() +
                ggtitle(paste(regioes_names[[i]]," - Produtividade de Milho por Região - Safras de 2000/01 a 2014/15"))
        ...
        qplot(Safra
              ...
              , ylab = "Produção (milhões de toneladas)") + 
                geom_line(position=pd,aes(group=UF)) +
                geom_point() +
                ggtitle(paste(regioes_names[[i]]," - Produção de Milho por Região - Safras de 2000/01 a 2014/15"))
